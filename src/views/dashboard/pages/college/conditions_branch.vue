...
<v-form ref="addconditions_transferform" lazy-validation>
            <v-card class="pa-2 ma-2" >
              <v-card-title>
                <div
                  class="font-weight-light v-size--x-large"
                ><v-icon large left>mdi-flag-letiant</v-icon> สาขาวิชาในการรับย้าย เรียงลำดับดังนี้</div>
              </v-card-title>              
              <v-col cols="12" sm="12">                   
                <v-card class="pa-2 ma-2">
                  <v-row>  
...
<v-row>  
   <v-col>
      ...
      <v-layout>
        <v-dialog v-model="addconditions_branchdialog" persistent max-width="80%">
          <v-card class="mx-auto pa-5">
            ...
            <v-form ref="addconditions_branchform" lazy-validation>
              <v-container grid-list-md>
                <v-layout wrap>
                   <v-flex md2>
<v-select v-model="addconditions_branch.sequence_n" :items="order_need" outlined   label="ลำดับที่ :">  
</v-select>
                   </v-flex>                                
                    <v-flex md6>                    
                      <v-autocomplete                         
      ...
      <v-layout>
        <v-dialog v-model="deletconditions_branchdialog" persistent max-width="40%">
          <v-card class="mx-auto pa-5">                     
             <base-material-card
              color="error"
              icon="mdi-delete"
          ...
          <v-card-text>
            <v-form ref="deleteconditions_branchform" lazy-validation>
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12>
                    ยืนยันการลบ <span> {{ editconditions_branch.id_branch }} </span> 
                  </v-flex>                                 
                </v-layout>
              </v-container>
            </v-form>
    ...
    async addconditions_branchSubmit() {
      ...
      async deleteconditions_branchsubmit() {
        if (this.$refs.deleteconditions_branchform.validate()) {
          ...
          let result = await this.$http.post('conditions_branch.delete.php', this.editconditions_branch)
          if (result.data.status == true) {
            this.conditions_branch = result.data
