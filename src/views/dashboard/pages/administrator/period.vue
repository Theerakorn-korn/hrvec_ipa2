        ...
        >
          ...
          <template v-slot:[`item.period_enable`]="{ item }">
            <v-icon
              ...
              v-if="item.period_enable === '1'"
              >mdi-alarm-light</v-icon
            >
            <v-icon v-if="item.period_enable === '0'"
           ...
           <template v-slot:[`item.period_enable_process`]="{ item }">
            <v-icon
              large
              color="green darken-2"
              v-if="item.period_enable_process === '1'"
              >mdi-alarm-light</v-icon
            >
            <v-icon v-if="item.period_enable_process === '0'"
              >mdi-alarm-light-outline</v-icon
            >
          </template>
          ...
          <template v-slot:[`item.period_type`]="{ item }">
            <span v-if="item.period_type === 'teacher'"
              >สายการสอนและสายสนับสนุน</span
            >
            <span v-if="item.period_type === 'manage'"
              >สายงานบริหารสถานศึกษา</span
            >
          </template>

          <template v-slot:[`item.actions`]="{ item }">
            <v-icon large color="yellow" @click.stop="periodEdit(item.id_pr)">
              mdi-pencil
            </v-icon>
          </template>
          <template v-slot:[`item.action_s`]="{ item }">
            <v-icon color="red" large @click.stop="periodDelete(item.id_pr)">
              mdi-delete
            </v-icon>
          </template>
          <v-alert
                  ...
                  <v-layout wrap>
                    ...
                    <v-flex xs12 md6>
                      <v-text-field
                        type="number"
                        label="ครั้งที่"
                        v-model="addperiod.period_times"
                        required
                        :rules="[v => !!v || '']"
                      ></v-text-field>
                    </v-flex>
                    ...
                    <v-flex xs12 md6>
                      <v-text-field
                        type="date"
                        label="วันที่เริ่ม"
                        v-model="addperiod.period_start"
                        required
                        :rules="[v => !!v || '']"
                      ></v-text-field>
                    </v-flex>
                    ...
                    <v-flex xs12 md6>
                      <v-text-field
                        ...
                        v-model="addperiod.period_cal_end"
                        required
                        :rules="[v => !!v || '']"
                      ></v-text-field>
                    </v-flex>
                ...
                <v-card-text>
                  <v-form ref="deleteperiodform" lazy-validation>
                    <v-container grid-list-md>
                      <v-layout wrap>
                        <v-flex xs12>
                          ยืนยันการลบรายการ {{ editperiod.id_pr }} :
                          {{ editperiod.period_year }}
                        </v-flex>
                      </v-layout>
                    </v-container>
                  </v-form>
